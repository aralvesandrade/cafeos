...
func TestPlotService_Create_Validation(t *testing.T) {
	...
	_, err := svc.Create(&entity.Plot{TenantID: "t1", FarmID: "f1", Name: "", Cultivar: "Catuaí", SoilType: "Argiloso", AreaHA: 10, PlantingYear: 2020, Altitude: 950})
	if err == nil {
		t.Error("expected error for empty name")
	}

	_, err = svc.Create(&entity.Plot{TenantID: "t1", FarmID: "f1", Name: "Plot", Cultivar: "Catuaí", SoilType: "Argiloso", AreaHA: 0, PlantingYear: 2020, Altitude: 950})
	if err == nil {
		t.Error("expected error for zero area")
	}

	_, err = svc.Create(&entity.Plot{TenantID: "t1", FarmID: "f1", Name: "Plot", AreaHA: 10, Stage: "invalid"})
	if err == nil {
...
func TestPlotService_ListByFarm(t *testing.T) {
	...
	svc.Create(&entity.Plot{TenantID: "t1", FarmID: "farm-1", Name: "Plot A", Cultivar: "Catuaí", AreaHA: 10, PlantingYear: 2020})
	svc.Create(&entity.Plot{TenantID: "t1", FarmID: "farm-1", Name: "Plot B", Cultivar: "Mundo Novo", AreaHA: 15, PlantingYear: 2019})
	svc.Create(&entity.Plot{TenantID: "t1", FarmID: "farm-2", Name: "Plot C", Cultivar: "Catuaí", AreaHA: 5, PlantingYear: 2021})
...
func TestPlotService_Update(t *testing.T) {
	...
	plot, _ := svc.Create(&entity.Plot{TenantID: "t1", FarmID: "f1", Name: "Old", Cultivar: "Catuaí", AreaHA: 10, PlantingYear: 2020, Altitude: 900})
	plot.Name = "Updated"
	err := svc.Update(plot)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
...
func TestPlotService_Update_InvalidDates(t *testing.T) {
	...
	plot, _ := svc.Create(&entity.Plot{TenantID: "t1", FarmID: "f1", Name: "Plot", AreaHA: 10})
