  ...
  return (<div className="space-y-6">
    ...
    <Table>
      ...
      <TableBody>{items.map((cc) => (<TableRow key={cc.id}>
        ...
        <TableCell className="font-medium"><div className="flex items-center gap-2"><CircleDollarSign className="h-4 w-4 text-primary" />{cc.name}</div></TableCell>
    ...
    <Dialog open={dialogOpen} onClose={() => { setDialogOpen(false); setEditing(null) }} title={editing ? 'Editar Centro de Custo' : 'Novo Centro de Custo'}>
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Categoria SENAR (opcional)</label>
          ...
          <p className="text-xs text-muted-foreground mt-1">Selecionar uma categoria preenche nome e classificação automaticamente.</p>
        </div>
        ...
        <div><label className="block text-sm font-medium text-foreground mb-1">Código</label><Input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="EX: DESP_ADUBOS" required /></div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium text-foreground mb-1">Classificação (COE/COT/CT)</label>
