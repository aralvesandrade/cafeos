    ...
    return (
      <Dialog open={open} onClose={handleClose} title="Criar Conta">
        <form onSubmit={handleSubmit} className="space-y-4">
          {plan && (
            <div className="bg-coffee-beige rounded-lg px-4 py-3 text-sm">
              <span className="text-coffee-text-light">Plano selecionado: </span>
              <span className="font-semibold text-coffee-green-dark">{plan}</span>
            </div>
          )}
      ...
      {step === 'channel' ? (
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center gap-4 p-4 rounded-lg border border-gray-200 hover:border-coffee-green/50 hover:bg-coffee-beige transition-colors text-left"
          >
            <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
              <MessageCircle className="h-5 w-5 text-green-600" />
            </div>
            <div>
              <p className="font-medium text-coffee-green-dark">WhatsApp</p>
              <p className="text-xs text-coffee-text-light">Atendimento rápido via mensagem</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <p className="font-medium text-coffee-green-dark">Email</p>
              <p className="text-xs text-coffee-text-light">Respondemos em até 24 horas</p>
            </div>
          </button>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <button
            ...
            className="flex items-center gap-1 text-sm text-coffee-text-light hover:text-coffee-green transition-colors mb-2"
          >
            <ChevronLeft className="h-4 w-4" />
            Voltar
          </button>

          <div className="flex items-center gap-2 text-sm text-coffee-green-dark mb-2">
            {channel === 'whatsapp' ? (
          ...
          <div>
            <label className="block text-sm font-medium text-coffee-text mb-1">
              {channel === 'whatsapp' ? 'WhatsApp' : 'Email'}
            </label>
            {channel === 'whatsapp' ? (
              <input
